...
class def extends site
{
	
	function exe()
	{
		$this->temps->get_templates('Front');
		
		$articles = '';
		$q = $this->db->q("SELECT * FROM {$this->db->pre}news WHERE visible=1 ORDER BY post_date DESC");
		while($r = $this->db->nqf($q))
		{
			$r['post_date'] = strtotime($r['post_date']);
			$articles .= eval($this->temps->d('NEWS_ARTICLE'));
		}
		$out = eval($this->temps->d('FRONT_LAYOUT'));
