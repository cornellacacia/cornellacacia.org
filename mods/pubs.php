...
class pubs extends site
{
	
	function exe()
	{
		$this->temps->get_templates('Pubs');
		
		$pubs = '';
		$q = $this->db->q("SELECT * FROM {$this->db->pre}pubs ORDER BY pub_order DESC");
		while($r = $this->db->nqf($q))
		{
			$pubs .= eval($this->temps->d('PUBLICATION'));
		}
